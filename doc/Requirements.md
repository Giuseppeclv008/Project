...
|-------------------------------|----------------------------------------------------------------|-------------|
| UC1 – Manage Inventory        | Create, modify, and delete batches                             | The owner manages inventory batches through CRUD operations. The system validates input, checks for existing batches, updates stored batch data, and handles duplicate-batch errors. |
| UC2 – Manage Supplier         | Create, modify, and delete suppliers                           | The owner manages suppliers through CRUD operations. The system checks for duplicates before committing any action. |
| UC3 – Manage Orders           | Create, modify, and delete purchase orders                     | The owner manages orders through CRUD operations. The system checks for duplicates before committing any action. |
| UC4 – Manage Invoices         | Create, modify, and delete invoices                            | The owner manages invoices through CRUD operations. The system checks for duplicates before committing any action. |
| UC5 – Authenticate Owner      | Verify owner identity to allow system access                   | The system prompts the owner for a password, verifies its correctness, and either grants or denies access based on the password's validity. |
| UC6 – Change Password         | Set or change the system password                              | The authenticated owner sets or updates the system's password. The system opens a data-entry dialog, validates the new password, and saves it securely. |
| UC7 – Manage Product Catalogue| Create, modify, and delete products                            | The owner manages products in the catalogue through CRUD operations. The system checks for duplicates before committing any action.|
| UC8 – Receive Notifications   | Notify the owner about relevant system events                  | The system generates notifications when one of the following events happens: order status updates are detected, batches expire, products fall under a certain threshold, cash registers become unresponsive, connectivity is lost or API crashes. Notifications are stored, displayed, read, and optionally dismissed by the owner. |
| UC9 – Import Data             | Import CSV data into the system                                | The owner imports `.csv` files for products, batches, sales, refunds, orders, suppliers, invoices, incomes, expenses, or shipping companies. The system validates the file's format and schema, detects errors, and imports data only when files are valid. |
| UC10 – Export Data            | Export system data as CSV files                                | The owner exports lists of products, batches, orders, suppliers, invoices, sales, refunds, incomes, expenses or shipping companies to a `.csv` file. The system retrieves data, generates `.csv` output, handles export errors, and delivers files for download. |
| UC11 – Retrieve Data          | Retrieve filtered or ranked lists of system data               | The owner requests lists filtered by selected attributes or ranked by specific metrics (e.g., top-selling products). The system opens a filter dialog, applies filters or ranking rules, retrieves data, and displays the resulting list. |
| UC12 – Manage Accounting      | Retrieve incomes, expenses, and balance                       | The owner requests financial data over a chosen time window and granularity. The system computes incoming and outgoing balance values and returns them to the owner. |
| UC13 – Manage Cash Registers  | Add, modify, or remove cash registers and synchronize data     | The owner links new cash registers through POS provider authentication, edits their attributes, or deletes them. The system validates POS provided tokens. |
| UC14 – Manage Sales and Refunds | Poll cash registers to acquire sales and refunds              | The system every $\approx 2$ minutes polls cash registers, retrieves sales and refund data, stores them, updates inventory quantities, and handles corrupted or unreadable data by discarding it safely. |
| UC15 – Get Catalogue          | Synchronize catalogue in the cash registers                      | The system checks synchronization status, retrieves the latest catalogue, and pushes updates to cash registers every morning at 6:00 AM. Unresponsive cash registers are detected and marked. |
| UC16 – Track Orders           | Update order status using shipment tracking services           | The system links with the tracking provider via authorization, sends tracking requests, receives status updates, applies changes to order's status, or handles unreachable tracking services without altering the stored status. |
| UC17 – Manage Shipping Companies | Create, modify, and delete shipping companies               | The owner manages shipping companies through CRUD operations. The system checks for duplicates before committing any action. |
