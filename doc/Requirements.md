...
|------|---------------|-------------|-----------|
| NFR1 | Functionality | **Suitability**<br>• All owner needs identified in the use cases should map to at least one functional requirement<br><br>**Accuracy**<br>• Catalogue updates, order-status updates, sales, refunds and similar synchronization operations must achieve a guaranteed delivery rate greater than 99.999 %<br>• Exported `.csv` files should have less than 1 mistaken row every 100'000<br>• Importing `.csv` files should generate less than 1 mistaken row every 100'000<br>• CRUD operations should have less than 1 error every 100'000 operations<br>• Notification should fail to trigger less than 1 every 100'000 expected notifications<br>• Order status should fail to update properly less than 1 time every 100'000 updates<br><br>**Interoperability**<br>• The application should use Square, Lightspeed, Clover, Nexi POS APIs<br>• The application should use the EasyPost API for tracking<br>• All functions accessing data concurrently must be thread-safe<br><br>**Security**<br>• Passwords must be encrypted using AES-256<br>• The application must pass GitLab SATS without high severity issues | FR1–FR14 |
| NFR2 | Reliability | **Maturity**<br>• The application should crash less than 1 time every 200 h<br>• Less than 1 minor glitch every 50 h<br><br>**Fault Tolerance**<br>• Must not crash on malformed `.csv` files<br>• Must continue operating offline except for sync operations<br>• Must handle corrupted/absent data from POS or shipping APIs<br>• Failed API calls must be queued and retried<br>• Multi-step functions must be atomic | FR1.2, FR2.2, FR3.4, FR4.3, FR5.3, FR6.3, FR7.3, FR9.3, FR10.2, FR10.4, FR8, FR13, FR14 |
| NFR3 | Usability | **Understandability**<br>• GUI must show tooltips<br>• Consistent design<br>• WCAG compliance<br><br>**Learnability**<br>• User can learn key actions within minutes<br><br>**Operability**<br>• CRUD operations within 2 clicks<br>• GUI responds within 500 ms | FR1–FR7, FR9, FR10, FR11, FR12, FR13 |
| NFR4 | Efficiency | **Time Behavior**<br>• CRUD operations ≤ 1000 ms<br>• Filtering ~100 MB ≤ 10 s<br>• CSV import/export ≤ 10 s per 1000 rows<br>• Order status updated within 3 h<br>• Sales/refunds updated every 2 min<br>• Catalogue sync daily at 6:00<br>• Startup ≤ 5 s<br><br>**Resources**<br>• Disk ≤ 500 MB<br>• RAM ≤ 2 GB<br>• CPU ≤ 50% | FR1.1, FR2.1, FR3.3, FR4.2, FR5.2, FR6.2, FR7.2, FR9.2, FR1.2, FR2.2, FR3.4, FR4.3, FR5.3, FR6.3, FR7.3, FR9.3, FR10.2, FR10.4, FR8, FR13, FR9.4 |
| NFR5 | Maintainability | **Analyzability**<br>• 95% code documented, logs, diagnostics<br><br>**Changeability**<br>• Minor features ≤ 1 week<br>• Major features ≤ 1 month<br><br>**Stability**<br>• Less than 1 regression per release<br><br>**Testability**<br>• Unit tests ≥ 90%<br>• Integration tests ≥ 80% | FR1–FR14 |
| NFR6 | Portability | **Adaptability**<br>• Runs on Windows 10/11<br><br>**Installability**<br>• Install completes ≤ 10 min<br>• Success ≥ 95%<br><br>**Co-existence**<br>• Works alongside antivirus and firewalls<br><br>**Interoperability**<br>• New versions must import previous `.csv` formats | FR1–FR13, FR11, FR14 |
