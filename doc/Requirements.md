...
| UC2 – Manage Supplier         | Create, modify, and delete suppliers                           | The owner manages suppliers. The system allows creation, update, and deletion of suppliers and  checks for duplicates. |
| UC3 – Manage Orders           | Create, modify, and delete purchase orders                     | The owner performs CRUD operations on orders and the system checks for duplicates |
| UC4 – Manage Invoices         | Create, modify, and delete invoices                            | The owner creates, updates, and deletes invoices. The system performs duplicate checks. |
| UC5 – Authenticate Owner      | Verify owner identity to allow system access                   | The system prompts the owner for a password, verifies correctness, grants access on success, and rejects access if the password is incorrect. |
| UC6 – Change Password         | Set or change the system password                              | The authenticated owner sets or updates the system password. The system opens a data-entry dialog, validates the new password, and saves it securely. |
| UC7 – Manage Product Catalogue| Create, modify, and delete products                            | The owner performs CRUD operations on products in the catalogue. The system checks for existing products and handles duplicate-product errors. |
| UC8 – Receive Notifications   | Notify the owner about relevant system events                  | The system generates notifications when detecting order status updates, expired batches, low-stock products, unresponsive cash registers, connectivity loss, or API failures. Notifications are stored, displayed, read, and optionally cleared by the owner. |
| UC9 – Import Data             | Import CSV data into the system                                | The owner imports `.csv` files for products, batches, sales, refunds, orders, suppliers, invoices, incomes, expenses, and shipping companies. The system validates file format and schema, detects errors, and imports data only when files are valid. |
| UC10 – Export Data            | Export system data as CSV files                                | The owner exports lists (products, batches, orders, suppliers, invoices, sales, refunds, incomes, expenses, shipping companies) to CSV. The system retrieves data, generates CSV output, handles export errors, and delivers files for download. |
| UC11 – Retrieve Data          | Retrieve filtered or ranked lists of system data               | The owner requests lists filtered by selected attributes or ranked by specific metrics (e.g., top-selling products). The system opens a filter dialog, applies filters or ranking rules, fetches data, and displays the resulting list. |
...
| UC13 – Manage Cash Registers  | Add, modify, or remove cash registers and synchronize data     | The owner links new cash registers through POS provider authentication, edits their attributes, or deletes them. The system validates tokens. |
...
| UC15 – Get Catalogue          | Synchronize catalogue in the cash registers                      | The system checks synchronization status, retrieves the latest catalogue, and pushes updates to cash registers. If a device does not respond, the system marks it as unresponsive. |
| UC16 – Track Orders           | Update order status using shipment tracking services           | The system links with the tracking provider via authorization, sends tracking requests, receives status updates, applies changes to order status, or handles unreachable tracking services without altering the stored status. |
| UC17 – Manage Shipping Companies | Create, modify, and delete shipping companies               | The owner manages shipping companies. The system allows creation, update, and deletion and checks for duplicates. |
