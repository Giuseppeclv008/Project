...
| UC14 – Manage Sales and Refunds | Poll cash registers to acquire sales and refunds              | The system periodically polls cash registers, retrieves sales and refund data, stores them, updates inventory quantities, and handles corrupted or unreadable data by discarding it safely. |
| UC15 – Get Catalogue          | Synchronize catalogue in the cash registers                      | The system checks synchronization status, retrieves the latest catalogue, and pushes updates to cash registers. Unresponsive cash registers are detected and marked. |
